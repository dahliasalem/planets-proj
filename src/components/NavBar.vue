...
<template>
    <nav class="flex flex-row sm:max-lg:flex-col items-center justify-between border-b border-white border-opacity-20 uppercase">
        <div class="flex font-antonio text-3xl sm:text-4xl p-4">
            The Planets
        </div>
        ...
        <div class="hidden sm:flex flex-row text-gray text-sm pr-4">
            ...
            <router-link class="p-3 block lg:inline-block lg:mt-0  hover:text-white focus:text-white py-8 border-t-4 border-t-red border-opacity-0 hover:border-opacity-100" :class="isCurrentRoute('mars') ? 'border-opacity-100':''" to="mars">Mars</router-link>
            <router-link class="p-3 block lg:inline-block lg:mt-0  hover:text-white focus:text-white py-8 border-t-4 border-t-yellow border-opacity-0 hover:border-opacity-100" :class="isCurrentRoute('jupiter') ? 'border-opacity-100':''" to="jupiter">Jupiter</router-link>
